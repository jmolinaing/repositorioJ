...
/* =======================================================================================  
    ...
    Modificado      : Jorge Molina <jorge.molina@nuevamasvida.cl>  
    fecha           : 04-02-2026  
    Descripción     : Req#17514  
                      Se modifica calculo @ult_periodo_cc Periodo Contable cerrado  
...
alter PROCEDURE [dbo].[spu_ges_cob_cju_ges_deuda] @p_cod_cob numeric(4), @p_ind_rjud char(1)  , @excluir_mediatica char(1) = null, @excluir_quiebra char(1) = null, @excluir_venta char(1) = null, @excluir_afiliados char(1) = null  
AS    
BEGIN     
 SET NOCOUNT ON;    
...
-- Optimized version: Replace correlated subqueries with CTEs and pre-calculated aggregates  
-- PERFORMANCE IMPROVEMENTS:  
-- 1. Eliminates 3 correlated subqueries (executed once per row)  
-- 2. Pre-calculates aggregates using CTEs  
-- 3. Consolidates deuda/DNP logic into single joins  
-- 4. Replaces CASE EXISTS with simpler JOIN logic  
  
WITH deuda_agg AS (  
    -- Pre-calculate debt by rut/cot/periodo  
    SELECT   
        DEC_RUT,  
        COT_RUT,  
        DEC_PERIODO,  
        SUM(DEC_PACTADO - DEC_PAGADO) AS deuda_total  
    FROM DEUDA_COTIZANTE WITH (NOLOCK)  
    GROUP BY DEC_RUT, COT_RUT, DEC_PERIODO  
),  
dnp_agg AS (  
    -- Pre-calculate DNP by epa_rut/cot/periodo  
    SELECT   
        DNP.EPA_RUT,  
        DD.COT_RUT,  
        DD.PPC_PERIODO,  
        SUM(DD.DDT_SALDO) AS dnp_total  
    FROM DNP WITH (NOLOCK)  
    INNER JOIN DNP_DETALLE DD WITH (NOLOCK)   
        ON DD.PPC_FOLIO = DNP.PPC_FOLIO  
    GROUP BY DNP.EPA_RUT, DD.COT_RUT, DD.PPC_PERIODO  
),  
rjud_exists AS (  
    -- Pre-check RJ existence  
    SELECT DISTINCT DDR_RUT, 1 AS has_rj  
    FROM RESOLUCION_JUDICIAL WITH (NOLOCK)  
),  
venta_cuest AS (  
    -- Pre-identify disputed sales  
    SELECT DISTINCT c.COT_RUT  
    FROM dbo.CONTRATO c WITH (NOLOCK)  
    INNER JOIN dbo.FUN f WITH (NOLOCK)   
        ON f.CON_FOLIO = c.CON_FOLIO   
        AND f.FUN_TIPO LIKE '%1%'  
    INNER JOIN dbo.Gescob_ejecutivo_cuestionado g WITH (NOLOCK)  
        ON g.EJE_RUT = f.EJE_RUT  
    WHERE c.CON_ULTIMO = 'S'  
),  
ult_pago AS (  
    -- Consolidate max payment date from both sources  
    SELECT   
        EPA_RUT,  
        MAX(ppc_fecha_pago) AS ppc_fecha_pago  
    FROM (  
        SELECT EPA_RUT, MAX(PPC_FECHA_PAGO) AS ppc_fecha_pago  
        FROM dbo.COTIZACION_PAGADA WITH (NOLOCK)  
        GROUP BY EPA_RUT  
        UNION ALL  
        SELECT EPA_RUT, MAX(PPC_FECHA_PAGO)  
        FROM dbo.PLANILLA WITH (NOLOCK)  
        GROUP BY EPA_RUT  
    ) t1  
    GROUP BY EPA_RUT  
)  
SELECT   
    ...
    UPPER(RTRIM(LTRIM(mdeu.DDR_NOMBRE))) AS deudor_nombre,  
    odasgper.DEC_PERIODO,  
    ISNULL(da.deuda_total, 0) AS deuda,  
    ISNULL(dn.dnp_total, 0) AS dnp,  
    CASE WHEN COALESCE(odasgper.DEC_TIPO_DEUDA, 'NP') = 'NP'   
         THEN COALESCE(odasgper.DEC_DEUDA, 0)   
         ELSE 0   
    END AS deuda_monto_np,  
    CASE WHEN COALESCE(odasgper.DEC_TIPO_DEUDA, 'NP') = 'DNP'   
         THEN COALESCE(odasgper.DEC_DEUDA, 0)   
         ELSE 0   
    END AS deuda_monto_dnp,  
    odasg.DEU_CORREL AS deu_correl,  
    UPPER(RTRIM(LTRIM(mcob.COB_NOMBRE))) AS cob_nom,  
    CASE WHEN rj.has_rj = 1 THEN 1 ELSE 0 END AS cob_rju_vig,  
    mdeu.DDR_MEDIATICA,  
    mdeu.DDR_QUIEBRA,  
    CASE WHEN vc.COT_RUT IS NOT NULL THEN 'X' ELSE '' END AS venta_cuestionada,  
    up.PPC_FECHA_PAGO AS ult_fecha_pago,  
    ISNULL(d5f.deuda_sum, 0) AS deuda5anos  
INTO #deuda_pre  
FROM COBRADOR AS mcob WITH (NOLOCK)  
INNER JOIN DEUDOR_ASIGNADO AS odasg WITH (NOLOCK)   
    ON odasg.COB_CODIGO = mcob.COB_CODIGO   
    AND odasg.DEU_ASIG_DESDE < GETDATE()   
    AND (odasg.DEU_ASIG_HASTA >= GETDATE() OR odasg.DEU_ASIG_HASTA IS NULL)  
INNER JOIN DEUASIG_PERIODO AS odasgper WITH (NOLOCK)   
    ON odasgper.DEU_CORREL = odasg.DEU_CORREL  
INNER JOIN DEUDOR AS mdeu WITH (NOLOCK)   
    ON mdeu.DDR_RUT = odasg.DDR_RUT  
LEFT JOIN deuda_agg AS da   
    ON da.DEC_RUT = odasg.DDR_RUT   
    AND da.COT_RUT = odasgper.COT_RUT   
    AND da.DEC_PERIODO = odasgper.DEC_PERIODO  
LEFT JOIN dnp_agg AS dn   
    ON dn.EPA_RUT = odasg.DDR_RUT   
    AND dn.COT_RUT = odasgper.COT_RUT   
    AND dn.PPC_PERIODO = odasgper.DEC_PERIODO  
LEFT JOIN rjud_exists AS rj   
    ON rj.DDR_RUT = mdeu.DDR_RUT  
LEFT JOIN venta_cuest AS vc   
    ON vc.COT_RUT = mdeu.DDR_RUT  
LEFT JOIN ult_pago AS up   
    ON up.EPA_RUT = mdeu.DDR_RUT  
LEFT JOIN #deuda5_filtrada AS d5f   
    ON d5f.DEC_RUT = mdeu.DDR_RUT  
WHERE mcob.COB_JUDICIAL = 'S'  
    AND (mcob.COB_CODIGO = @p_cod_cob OR @p_cod_cob IS NULL)  
    AND (@p_ind_rjud = 'N' OR NOT EXISTS (  
        SELECT 1 FROM DEUDA_COTIZANTE dc WITH (NOLOCK)   
        WHERE dc.DEC_RUT = odasg.DDR_RUT   
        AND dc.COT_RUT = odasgper.COT_RUT   
        AND dc.DEC_PERIODO = odasgper.DEC_PERIODO   
        AND DEC_NRORESOL > 0  
    ))  
    AND ((@excluir_mediatica = 'S' AND (mdeu.DDR_MEDIATICA = 'N' OR mdeu.DDR_MEDIATICA IS NULL))  
        OR @excluir_mediatica = 'N'   
        OR @excluir_mediatica IS NULL)  
    AND ((@excluir_quiebra = 'S' AND (mdeu.DDR_QUIEBRA = 'N' OR mdeu.DDR_QUIEBRA IS NULL))  
        OR @excluir_quiebra = 'N'   
        OR @excluir_quiebra IS NULL);  
