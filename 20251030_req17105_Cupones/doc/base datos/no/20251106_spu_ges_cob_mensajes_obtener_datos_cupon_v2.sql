/*========================================================================================   
 ...
 descripción  : Tareas #17105, se cambio temporalmente , para pruebas, el nombre a spu_ges_cob_mensajes_obtener_datos_cupon
 - se agrego la columna CUP_CORREL numeric(15) a #tabla_final
 - se crea nueva tabla temporal #tabla_final_filtrada, que es copia de la estructura de la tabla #tabla_final
...
--execute spu_ges_cob_mensajes_obtener_datos 1  --167.931reg 2min
execute spu_ges_cob_mensajes_obtener_datos 1, 'S', 5,  @epl_fechora  --167.931reg 2min
--execute spu_ges_cob_mensajes_obtener_datos 2   --166.382reg 2min

execute spu_ges_cob_mensajes_obtener_datos_cupon 1 con #tabla_final_filtrada   --167.931reg 2min
execute spu_ges_cob_mensajes_obtener_datos_cupon 2   --166.382reg 2min
execute spu_ges_cob_mensajes_obtener_datos_cupon 1, 'S', 5, @epl_fechora , con #tabla_final_filtrada   --167.931reg 2min
...
alter procedure [dbo].[spu_ges_cob_mensajes_obtener_datos_cupon]   
...
BEGIN   
 ...
 BEGIN  
  ...
  BEGIN  
   IF @NOMBRE_VALOR LIKE '%1%'  
    SET @sqlfiltro = CASE WHEN @sqlfiltro = '' THEN ' and EDAD_DEUDOR >= 18 and EDAD_DEUDOR <= 25 ' ELSE @sqlfiltro + ' and EDAD_DEUDOR >= 18 and EDAD_DEUDOR <= 25 ' END  
   IF @NOMBRE_VALOR LIKE '%2%'  
    SET @sqlfiltro = CASE WHEN @sqlfiltro = '' THEN ' and EDAD_DEUDOR >= 26 and EDAD_DEUDOR <= 40 ' ELSE @sqlfiltro + ' and EDAD_DEUDOR >= 26 and EDAD_DEUDOR <= 40 ' END  
   IF @NOMBRE_VALOR LIKE '%3%'  
    SET @sqlfiltro = CASE WHEN @sqlfiltro = '' THEN ' and EDAD_DEUDOR >= 41 and EDAD_DEUDOR <= 55 ' ELSE @sqlfiltro + ' and EDAD_DEUDOR >= 41 and EDAD_DEUDOR <= 55 ' END  
   IF @NOMBRE_VALOR LIKE '%4%'  
    SET @sqlfiltro = CASE WHEN @sqlfiltro = '' THEN ' and EDAD_DEUDOR < 18 and EDAD_DEUDOR > 55 ' ELSE @sqlfiltro + ' and EDAD_DEUDOR < 18 and EDAD_DEUDOR > 55 ' END  
 ...
 ( rut_deudor char(10) not null  
  ...
  , cup_correl numeric(15) null		--Tareas #17105
  --, primary key (rut_deudor)  
 )  

--Tareas #17105Se crea #tabla_final_filtrada, copia en estructura de #tabla_final, y contendra los registros filtrados.
 select *
...
--Tareas #17105: CUPONES ______________________________________________________
	...
	--select * from #tabla_final_filtrada
	--return 
