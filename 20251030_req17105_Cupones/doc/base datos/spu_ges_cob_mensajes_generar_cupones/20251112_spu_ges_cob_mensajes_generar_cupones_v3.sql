...
IF NOT EXISTS(
	SELECT * FROM [150.10.11.55].[ISAPRE].DBO.GCO_ENVMSG_DEUDA_CUPONES WHERE CUP_ID_BASE = @cup_id_base
)
BEGIN
   RAISERROR('No existe GCO_ENVMSG_DEUDA_CUPONES: ', 16, 1)  
   RETURN  
END
    ...
    --,[FECHA_FUN_HAB]  
    --,[REJ_FOLIO]  
    FROM [150.10.11.55].[ISAPRE].DBO.GCO_ENVMSG_DEUDA_CUPONES  
...
--JMOLINA, CREE ESTA SECCION V1 PARA REEMPLAZAR LA  SECCION V0 
  ...
  /*COMENTE ESTO JMOLINA POR QU ENO EXISTE PARAMETRO epa_rut 
 ...
 INSERT INTO #tmp_cuponpago_cotiz  
  ...
  SELECT   ROW_NUMBER() over (PARTITION BY '' ORDER BY tmp.epa_rut, ep.epa_razon) +  @cup_correl - 1,  
	 ...
	 GETDATE(), --JMOLINA, ERROR EN EL ORIGEN FALTA ESTE CAMPO
     tmp.epa_rut,  
