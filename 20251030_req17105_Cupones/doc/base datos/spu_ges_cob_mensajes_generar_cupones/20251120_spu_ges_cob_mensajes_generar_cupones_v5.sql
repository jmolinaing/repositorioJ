...
========================================================================================*/  
--select * from PLANILLA_PAGO_COTIZ where
--GRANT EXECUTE ON OBJECT::dbo.spu_ges_cob_mensajes_generar_cupones TO PUBLIC;
/*
declare @getdate datetime
set @getdate = getdate()
EXECUTE spu_ges_cob_mensajes_generar_cupones @getdate '2025-11-14 13:05:52.433'
...
alter PROCEDURE [dbo].[spu_ges_cob_mensajes_generar_cupones]  
 ...
 --@correl_gescob numeric(10)=null,@correl_gescob --@valida_lagunas char(1)='S'
AS  
BEGIN   
 SET NOCOUNT ON; 
 SET XACT_ABORT ON
...
/*JMOINA, COMENTE TODA ESTA SECCION V0 
  --JMOLINA
  DECLARE @correl_gescob NUMERIC(15)-- LO CREE YO
  ...
  ELSE  
   BEGIN  
    ...
    ----------------------------------------------------------------------------------------------------------------  
 /* COMENTE ESTO JMOLINA, POR QU ENO EXISTE VARIABLE @valida_lagunas
  ...
  END			-- IF @correl_gescob IS NULL
   
JMOLINA, COMENTE TODA ESTA SECCION V0 */

    
--JMOLINA, CREE ESTA SECCION V1 PARA REEMPLAZAR LA  SECCION V0 
IF NOT EXISTS(
	SELECT * FROM [evolution].[ISAPRE].DBO.GCO_ENVMSG_DEUDA_CUPONES WHERE CUP_ID_BASE = @cup_id_base
)
BEGIN
   RAISERROR('No existe registros en GCO_ENVMSG_DEUDA_CUPONES', 16, 1)  
   RETURN  
END
...
--SELECT @CONTADOR = COUNT(*) FROM [150.10.11.55].[ISAPRE].DBO.GCO_ENVMSG_DEUDA_CUPONES WHERE CUP_ID_BASE=@cup_id_base

--PRINT '@CONTADOR:'+CAST(@CONTADOR AS VARCHAR(10))+'_'
--RETURN 
 ...
 delete from #consdeuda where pactado - pagado <=0					---** COMENTAR POR PRUEBA YA QUE LOS REGISTROS TRAEN CERO***************
  ...
  /*COMENTE ESTO JMOLINA ya QUE NO EXISTE PARAMETRO epa_rut 
  ...
  */
  ...
  /*COMENTE JMOLINA, YA QUE NO EXISTE PARAMTROS DE ENTRADA @desc_deudanom, @desc_reajuste, @desc_interes, @desc_recargo
  UPDATE #consdeuda  
...
--PRINT @cant_reg
--PRINT '@cant_reg:'+CAST(@cant_reg AS VARCHAR(20))+'_'
--RETURN --********************************************************************************************

--Jmolina: me di cuenta que si @cant_reg = 0 los sgtes dos insert se caen, ya que los sp debieran traer dos columnas pero si @cant_reg = 0 solo traen una columna.
-- esto es porque mas arriba esta esto :delete from #consdeuda where pactado - pagado <=0, para los registros de prueba daban cero
   if @cant_reg = 0
   begin   
      RAISERROR('No existe cantidad de registros para generar los FOLIOS DE PLANILLA', 16, 1)  
      RETURN  
   end
--
 ...
 INSERT INTO #tmp_cuponpago_cotiz  
  ...
  SELECT   ROW_NUMBER() over (PARTITION BY '' ORDER BY tmp.epa_rut, ep.epa_razon) +  @cup_correl - 1,  
	 ...
	 @cup_id_base,	--GETDATE(), --JMOLINA, ERROR EN EL ORIGEN FALTA ESTE CAMPO
     tmp.epa_rut,  
