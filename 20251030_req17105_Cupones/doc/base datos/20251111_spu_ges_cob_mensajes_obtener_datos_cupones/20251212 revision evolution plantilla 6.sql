...
declare @epl_codigo varchar(50) = null  
, @enviar char(1) = 'N'  
...
--grupo c deudas mayores a 2 uf
 ...
 if object_id('tempdb..#fono', 'u') is not null drop table #fono
  if object_id('tempdb..#TMP_DEUDA_FINANCIERA', 'u') is not null drop table #TMP_DEUDA_FINANCIERA
...
--select * from #tabla_final where gestion29 in ('No') and equipo_cobrador = 'INTERNO' AND TIPO_DEUDOR IN ('VIGENTE', 'NO VIGENTE')
...
BEGIN
    ...
    BEGIN
            ...
            BEGIN TRY
                BEGIN TRANSACTION
			        ...
			        BEGIN TRY
							...
							PRINT 'TRANCOUNT (SPPADRE): ' + CAST(@@TRANCOUNT AS VARCHAR(10))
			        ...
			        END TRY
					        BEGIN CATCH
						        IF @@TRANCOUNT > 0
							        ROLLBACK TRANSACTION;
							        SELECT @ErrorMessage = ERROR_MESSAGE()
							        RAISERROR('(1)Fallo la generación de cupones con el SP spu_ges_cob_mensajes_generar_cupones: %s', 16, 1, @ErrorMessage)
							        RETURN
					        END CATCH
            ...
            BEGIN CATCH
                ...
                --RAISERROR(@ErrorMessage, @ErrorSeverity, @ErrorState)
		        RAISERROR('Error proceso GENERACIÓN CUPONES DEUDA COTIZACIONES: %s', @ErrorSeverity, @ErrorState, @ErrorMessage)
            END CATCH
    ...
    BEGIN
            ...
            BEGIN CATCH
                ...
                --RAISERROR(@ErrorMessage, @ErrorSeverity, @ErrorState)
		        RAISERROR('Error proceso GENERACIÓN CUPONES DEUDA FINANCIERA: %s', @ErrorSeverity, @ErrorState, @ErrorMessage)
            END CATCH
