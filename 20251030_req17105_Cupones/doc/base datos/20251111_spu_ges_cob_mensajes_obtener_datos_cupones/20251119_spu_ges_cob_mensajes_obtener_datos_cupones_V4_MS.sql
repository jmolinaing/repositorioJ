...
execute spu_ges_cob_mensajes_obtener_datos_cupones 1, 'S', 5, @epl_fechora , 'S' 

execute spu_ges_cob_mensajes_obtener_datos 1
execute spu_ges_cob_mensajes_obtener_datos_cupones 1, 'N', NULL, NULL, 's'		--eran 1160 reg aprox en 45 seg aprox, EN TABLA NUEVA: 1265 
 ...
 if @enviar = 'S'  
  ...
  set @ate_codigo = 1 --PRUEBA **********************
   ...
   begin  
    ...
    --return   
     -- RAISERROR('No existe programa con api template con el código de programa(@epr_codigo): %i', 16, 1, @epr_codigo) 
   ...
   begin  
      RAISERROR('Falta parámetro de entrada @epl_fechora, para el código de programa(@epr_codigo): %i', 16, 1, @epr_codigo)  
      RETURN  
   end  
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION
			...
			BEGIN TRY
					...
					--select * from GCO_ENVMSG_DEUDA_CUPONES where CUP_ID_BASE = @getdate
					--return
    ...
    BEGIN CATCH
        ...
        --RAISERROR(@ErrorMessage, @ErrorSeverity, @ErrorState)
		RAISERROR('Error general en proceso de generación de cupones: %s', @ErrorSeverity, @ErrorState, @ErrorMessage)
    END CATCH
 ...
 IF @generar_cupones = 'S'
	BEGIN
		...
		BEGIN TRY  
			...
			WHERE CUP_ID_BASE < DATEADD(M,-3 , GETDATE())
		...
		BEGIN CATCH
			IF @@TRANCOUNT > 0
				ROLLBACK TRANSACTION;
				SELECT @ErrorMessage = ERROR_MESSAGE()
				RAISERROR('Fallo la eliminación de registros en GCO_ENVMSG_DEUDA_CUPONES: %s', 16, 1, @ErrorMessage)
				RETURN
		END CATCH
