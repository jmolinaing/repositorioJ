...
--GRANT EXECUTE ON [spu_ges_cob_mensajes_filtros_consultar] TO public;  
...
-- INSERT #TOTAL ______________________________  
 ...
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 1, NULL, 1, 'Equipo Interno' , NULL UNION    
    SELECT 1, NULL, 2, 'Equipo Stock' , NULL UNION    
    SELECT 1, NULL, 3, 'Equipo judicial' , NULL UNION    
    SELECT 1, NULL, 4, 'Otros' , NULL    
    
    
 --SUPERVISOR    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
 SELECT 2, NULL, S.SCO_CODIGO, S.USU_LOGIN, NULL    
 FROM DBO.SUPERVISOR_COB S WITH (NOLOCK)    
 WHERE EXISTS (SELECT * FROM USUARIO U WITH (NOLOCK)     
     WHERE USU_INIVIG <= GETDATE()
			AND (USU_FINVIG >= GETDATE() OR USU_FINVIG IS NULL ) AND S.USU_LOGIN = U.USU_LOGIN)    
    
 --Cobrador Asignado    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
SELECT 3, NULL, COB_CODIGO, COB_NOMBRE, NULL    
FROM COBRADOR WITH (NOLOCK)
WHERE COB_INIVIG <= GETDATE()
AND (COB_FINVIG >= GETDATE() OR COB_FINVIG IS NULL )
AND EXISTS (SELECT * FROM USUARIO 
			WHERE USU_INIVIG <= GETDATE()
			AND (USU_FINVIG >= GETDATE() OR USU_FINVIG IS NULL ) 
			AND COBRADOR.COB_RUT=USUARIO.USU_RUT ) 
    
 -- GRUPO o Equipo    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 4, NULL, 1, 'A: (DEUDA <= A UF 1)' , NULL UNION    
    SELECT 4, NULL, 2, 'B: (DEUDA <= A UF 2)' , NULL UNION    
   SELECT 4, NULL, 3, 'C: (DEUDA > A UF 3)' , NULL    
    
 -- Tipo y Vigencia Deudor  , ALEX  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 5, NULL, 1, 'Vigente' , NULL UNION    
    SELECT 5, NULL, 2, 'No Vigente' , NULL UNION    
    SELECT 5, NULL, 3, 'Empresa' , NULL    
    
 -- Con Gestión Cód.. 29    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 6, NULL, 1, 'Con' , NULL UNION    
    SELECT 6, NULL, 2, 'Sin' , NULL     
  
 -- Fecha Compromiso vencido    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 7, NULL, 1, 'Vencidos (Compromiso Hoy-1)' , NULL UNION    
    SELECT 7, NULL, 2, 'No Vencidos (Compromiso >=Hoy)' , NULL     
    
 -- Tipo de Deuda    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 8, NULL, 1, 'Cotizaciones' , NULL UNION    
    SELECT 8, NULL, 2, 'Ley de Urgencia' , NULL UNION    
    SELECT 8, NULL, 3, 'Cheques Protestados' , NULL    
  
 -- Tipo Deuda Cotizaciones   
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 9, NULL, 1, 'DNP' , NULL UNION    
    SELECT 9, NULL, 2, 'IP' , NULL UNION    
    SELECT 9, NULL, 3, 'DPP' , NULL    
  
 -- Menor Periodo de Deuda  ALEX  
 -- Marcelo: Menor periodo de deuda: hasta 6, hasta 5, hasta 4  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 10, NULL, 1, 'Hasta 6 Meses atrás' , NULL UNION    
    SELECT 10, NULL, 2, 'Hasta 5 Meses atrás' , NULL UNION    
    SELECT 10, NULL, 3, 'Hasta 4 Meses atrás' , NULL     
    --SELECT 10, NULL, 1, 'Mes (Hoy - 180)' , NULL UNION    
    --SELECT 10, NULL, 2, 'Mes (Hoy - 179)' , NULL UNION    
    --SELECT 10, NULL, 3, 'Mes (Hoy - 178)' , NULL UNION   
 --   SELECT 10, NULL, 4, 'Otros' , NULL    
  
 -- Mayor Periodo de Deuda  ALEX  
 -- Marcelo: Mayor periodo de deuda: hasta 4, Hasta 3, Hasta 2  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
 SELECT 11, NULL, 1, 'Hasta 4 Meses atrás' , NULL UNION    
 SELECT 11, NULL, 2, 'Hasta 3 Meses atrás' , NULL UNION    
 SELECT 11, NULL, 3, 'Hasta 2 Meses atrás' , NULL     
    --SELECT 11, NULL, 1, 'Mes (Hoy - 2)' , NULL UNION    
    --SELECT 11, NULL, 2, 'Mes (Hoy - 3)' , NULL UNION    
    --SELECT 11, NULL, 3, 'Mes (Hoy - 4)' , NULL UNION    
    --SELECT 11, NULL, 4, 'Otros' , NULL    
  
 --Ciudad de residencia del deudor  
 --select * from ciudad  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
 SELECT 12, NULL, CIU_CODIGO, CIU_NOMBRE, NULL    
 FROM CIUDAD WITH (NOLOCK) ORDER BY CIU_CODIGO    
  
  -- Deudores LUR con Crédito 5%    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 13, NULL, 1, 'SI' , NULL UNION    
    SELECT 13, NULL, 2, 'NO' , NULL     
  
  -- Tipo de Empresa  ALEX  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 14, NULL, 1, 'Publica' , NULL UNION    
    SELECT 14, NULL, 2, 'Privada' , NULL     
  
 -- Rubro Empresa ALEX  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 15, NULL, 1, 'Fundaciones' , NULL UNION    
    SELECT 15, NULL, 2, 'Corporaciones' , NULL UNION    
    SELECT 15, NULL, 3, 'Telecomunicaciones' , NULL UNION    
    SELECT 15, NULL, 4, 'Otras' , NULL  
  
  -- Posible Compensar x TFU    
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 16, NULL, 1, 'SI' , NULL UNION    
    SELECT 16, NULL, 2, 'NO' , NULL     
  
 -- Edad Deudor  ALEX  
 INSERT #TOTAL (CODIGO_CONCEPTO, NOMBRE_CONCEPTO, CODIGO_VALOR, NOMBRE_VALOR, SELECCIONADO)    
    SELECT 17, NULL, 1, '18-25' , NULL UNION    
    SELECT 17, NULL, 2, '26-40' , NULL UNION    
    SELECT 17, NULL, 3, '41-55' , NULL UNION    
    SELECT 17, NULL, 4, 'Otros' , NULL  
